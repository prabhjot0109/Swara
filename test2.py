...
def recordWin():
    ...
    def fileInput(b_x,b_y):
            ...
            if file:
                    ...
                    path = filePath
                    file_loc_l = Label(rec_root , text=str(filePath),font = "raleway 10 bold", bg="paleturquoise")
    ...
    def recording(sec_e):
            ...
            ch2 = Label(rec_root , text= "Your voice is Recorded!" , bg="paleturquoise" ,font= ("Posterama  16 bold"))
    ...
    # Record Audio button
    record_button = Button(rec_root, text="Record Audio", command= lambda : recording(sec_e) , font = " arial 15 bold", width = 14, height= 2,
...
def fileWin():
        ...
        #                 filePath = os.path.abspath(file.name)
        #                 path = filePath
        #                 file_loc_l = Label(f_root , text=str(filePath),font = "raleway 10 bold", bg="paleturquoise")
        #                 file_loc_l.place(x = b_x, y = (b_y + 30))                        
        #                 return path
        ...
        #         label_file_1.config(text=user_file)

        # def get_org_file():
        #         org_file = filedialog.askopenfilename()
        #         label_file_2.config(text= org_file)
        #         plot_audio_files()

        # def plot_audio_files():
        #         user_file = label_file_1.cget("text")
        #         org_file = label_file_2.cget("text")
        #         # Swara_Backend.plot_audio_files(user_file,  org_file)

        # # root = tk.Tk()

        # label_file_1 = tk.Label(f_root , text="")
        # label_file_1.pack()

        # button_1 = tk.Button(f_root, text="Select File 1", command=get_user_file)
        # button_1.pack()

        # label_file_2 = tk.Label(f_root, text="")
        # label_file_2.pack()

        # button_2 = tk.Button(f_root, text="Select File 2", command=get_org_file)
        # button_2.pack()
        
        # print(user_file)
        # print(org_file)
        # process_button = tk.Button(f_root, text="Process Files", command= plot_audio_files)
        # process_button.pack()
                # def get_file_path():
        ...
        # button = tk.Button(f_root, text="Select File", command=get_file_path)
        # button.pack()
        ...
        # button = tk.Button(f_root, text="Select File", command=get_file_path1)
        # button.pack()
        ...
        file1_label = Label(f_root, text="File 1")
        file1_label.pack()

        user_file = tk.StringVar()
        # user_file_loc = ""
        file1_entry = Entry(f_root, textvariable=user_file)
        file1_entry.pack()

        file1_button = Button(f_root, text="Select File", command=lambda: user_file.set(filedialog.askopenfilename()))
        file1_button.pack()

        # File 2 selection
        file2_label = Label(f_root, text="File 2")
        file2_label.pack()

        org_file = tk.StringVar()
        # orgMusic_file_loc = ""
        file2_entry = Entry(f_root, textvariable=org_file)
        file2_entry.pack()

        file2_button = Button(f_root, text="Select File", command=lambda: org_file.set(filedialog.askopenfilename()))
        file2_button.pack()
        
        # Plot and Compare button
        plot_and_compare_button = tk.Button(f_root, text="Plot and Compare", command=lambda: Swara_Backend.plot_audio_files(user_file.get(), org_file.get()) or Swara_Backend.similarity_and_pitch(user_file.get(), org_file.get()))
        ...
        # user_file_but = Button(f_root , text= "Browse",command=lambda : fileInput(30,85))
        ...
        # org_file_but = Button(f_root , text= "Browse",command=lambda : fileInput(30,148))
        # orgMusic_file_loc = fileInput(30,148)
...
def graphWin():
        ...
        try:    
                fig = Swara_Backend.plot_audio_files(user_file ,  org_file)
                Swara_Backend.similarity_and_pitch(result_text , user_file ,   org_file)
