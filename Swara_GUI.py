...
class Functionality:
    ...
    def fileInput(self, win, evar, b_x, b_y):
        self.file = filedialog.askopenfile(
            mode="r", filetypes=[("Music Files", "*.wav")]
        )

        if self.file:
            self.filePath = os.path.abspath(self.file.name)
            self.path = self.filePath
            self.fileLocationEntry = Entry(
                win, textvar=evar, font="raleway 10 bold", bg="#1b191a", bd=0, width=200
            )
    ...
    def recording(self, win, invervalSecEntry):
        ...
        for i in range(0, int(self.RATE / self.FRAMES_PER_BUFFER * self.seconds)):
            self.ch3 = Label(
                win,
                textvariable=f"Time Left: {self.seconds - self.second_count} seconds",
                bg="#1b191a",
            )
...
def loginWin():
    ...
    closeButton = Button(
        ...
        command=login,
    )  # Checks and verify the login details
...
def createUserWin():
    ...
    mobileNumberLabel = Label(
        detailsFrame, text="Mobile Number : ", bg="#1b191a", fg="#f3b32d"
    )
    mobileNumberLabel.grid(row=0, column=0, padx=5, pady=10)
    userCreateMobile.set("")

    mobileNumberEntry = Entry(
        detailsFrame, width=20, justify="center", textvariable=userCreateMobile
    )
    ...
    # E-mail
    emailLabel = Label(
        detailsFrame, text="Email Address : ", bg="#1b191a", fg="#f3b32d"
    )
    emailLabel.grid(row=1, column=0, padx=5, pady=10, sticky="w")
    userCreateEmail.set("")

    emailEntry = Entry(
        detailsFrame, width=20, justify="center", textvariable=userCreateEmail
    )
    ...
    usernameEntry = Entry(
        detailsFrame, width=20, justify="center", textvariable=userCreateUsername
    )
    ...
    passwordEntry = Entry(
        ...
        justify="center",
        show="*",
        textvariable=userCreatePassword,
    )
    ...
    firstNameLabel = Label(
        detailsFrame, text="First Name : ", bg="#1b191a", fg="#f3b32d"
    )
    firstNameLabel.grid(row=4, column=0, padx=5, pady=10, sticky="w")
    userCreateFirstName.set("")

    firstNameEntry = Entry(
        detailsFrame, width=20, justify="center", textvariable=userCreateFirstName
    )
    ...
    lastNameEntry = Entry(
        detailsFrame, width=20, justify="center", textvariable=userCreateLastName
    )
    ...
    def choose_date():
        ...
        # Ok Button
        ok = Button(
            datePicker, text="OK", bd=5, command=picked, fg="deeppink", bg="#141a1a"
        )
        ok.pack(pady=10)
    ...
    # Sumbit Button
    submit = lambda: database.regSumbit(
        signUpWin,
        userCreateEmail,
        userCreateMobile,
        userCreateUsername,
...
def recordWin():
    ...
    orgRecFileButton = Button(
        recordingWin, bg="#141a1a", fg="deeppink", text="Browse", command=orgRecFileLoc
    )  # (120,140))
    ...
    recordButton = Button(
        ...
        bg="#141a1a",
        fg="deeppink",
        command=lambda: functionality.recording(recordingWin, invervalSecEntry),
        font=" arial 15 bold",
        width=14,
        height=2,
        relief=RAISED,
...
def fileWin():
    ...
    instructionLabel = Label(
        fileInputWin,
        text="Input your recorded music file and Original Music File Here.",
        bg="#1b191a",
        fg="#f3b32d",
        font=("Posterama  12 bold"),
    )
    ...
    userFileButton = Button(
        fileInputWin,
        bg="#141a1a",
        fg="deeppink",
        text="Browse",
        command=lambda: functionality.fileInput(fileInputWin, userFileLocEntry, 30, 85),
    )
    ...
    orgFileButton = Button(
        fileInputWin,
        fg="deeppink",
        bg="#141a1a",
        text="Browse",
        command=lambda: functionality.fileInput(fileInputWin, orgFileLocEntry, 30, 148),
    )
    orgFileLoc = functionality.fileInput(fileInputWin, orgFileLocEntry, 30, 148)
    orgFileButton.place(x=140, y=148)

    # Plot and Compare button
    plotAndCompareButton = Button(
        fileInputWin,
        bg="#141a1a",
        fg="deeppink",
        text="Plot and Compare",
...
def graphWin():
    ...
    try:
        fig = backend.plotAudioFiles(
            grpWin, orgFileLocEntry, userFileLocEntry, userFileLoc
        )
    ...
    # Back Button
    backButton = Button(
        grpWin, text="<--- Go Back", bg="white", fg="deeppink", bd=6, command=back
    )
